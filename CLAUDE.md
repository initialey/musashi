...
現在の実装範囲は、採用下層ページ「会社を知る」の **全域（Hero〜フッター）**。
node `827:52372` の y0〜10647 を実装済み。ただし「6つの数字」の数値と
フッターの構造は未確定（`docs/design-decisions.md` の 2026-08-16
「チャンク1〜4」参照、Figma MCP 切断中に metadata のみで再構築した回）。
